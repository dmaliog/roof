        ...
        objectForm.addEventListener('submit', (e) => {
            ...
            expenseForm.addEventListener('submit', (e) => {
                ...
                manualPriceForm.addEventListener('submit', (e) => {
                    ...
                    // Добавьте эту функцию в конец вашего script.js файла

                    // Функция для исправления модальных окон в мобильных приложениях
                    function fixModalWindowsInWebView() {
                        // Получаем все модальные окна
                        const historyModal = document.getElementById('history-modal');
                        const forms = [
                            document.getElementById('object-form'),
                          document.getElementById('expense-form'),
                          document.getElementById('manual-price-form'),
                          document.getElementById('custom-service-form')
                        ].filter(form => form); // Фильтруем null значения

                        // Функция для принудительной установки размеров модального окна
                        function forceModalDimensions(modal) {
                            if (!modal) return;

                            // Получаем содержимое модального окна
                            const modalContent = modal.querySelector('.modal-content') || modal;

                            // Устанавливаем минимальную высоту и другие важные стили
                            ...
                            // Убедимся, что крестик закрытия виден
                            const closeButton = modal.querySelector('#close-history') || modal.querySelector('.close-btn');
                            if (closeButton) {
                        ...
                        // Исправляем историю модального окна
                        if (historyModal) {
                            // Переопределяем функцию отображения истории
                            const originalShowHistory = window.showHistory;
                            window.showHistory = function(e) {
                                if (originalShowHistory) {
                                    originalShowHistory.call(this, e);
                                }

                                // Принудительно устанавливаем размеры после открытия
                                setTimeout(() => {
                                    forceModalDimensions(historyModal);
                                }, 10);
                            };
                        ...
                        // Исправляем формы
                        forms.forEach(form => {
                            forceModalDimensions(form);

                                      // Добавляем стили для кнопок формы
                        ...
                        // Функция для проверки и исправления модальных окон при изменении размера экрана
                        ...
                        // Добавляем обработчики событий
                        window.addEventListener('resize', checkAndFixModals);
                        window.addEventListener('orientationchange', checkAndFixModals);

                        // Переопределяем функцию showForm для применения стилей при открытии форм
                        const originalShowForm = window.showForm;
                        if (typeof originalShowForm === 'function') {
                            window.showForm = function(formToShow) {
                                originalShowForm.call(this, formToShow);

                                if (formToShow) {
                                    setTimeout(() => {
                                        forceModalDimensions(formToShow);
                                    }, 10);
                                }
                            };
                        }
                    ...
                    document.addEventListener('DOMContentLoaded', fixModalWindowsInWebView);

                    // Также вызываем функцию сразу, если DOM уже загружен
                    if (document.readyState === 'interactive' || document.readyState === 'complete') {
                        fixModalWindowsInWebView();
                    }
